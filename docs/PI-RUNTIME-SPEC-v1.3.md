...
### 17.1 Storage Semantics

The production runtime replaces in-memory registries with persistent,
append-only SQLite storage:

- **Append-Only Constraint:** Tables `snapshots`, `audit_log`, `receipts`
  are append-only. UPDATE and DELETE are blocked by database triggers.
- **Hash Chaining:** Every snapshot row carries `previous_hash` and
  `artifact_hash`. Chain integrity is verifiable via `verify_chain()`.
- **Tenant Isolation:** Every table has `tenant_id` column with composite
  indexes. Cross-tenant queries are rejected at the API layer.
- **WAL Mode:** SQLite runs with `journal_mode = WAL` for concurrent reads.
- **Retention:** Configurable via `RetentionPolicy` (hot/warm/cold/archive).

### 17.2 Security Semantics

- **Authentication:** JWT with HMAC-SHA256. Claims include `tenant_id`,
  `role`, `sub`. Tokens expire deterministically.
- **Request Signing:** HMAC-SHA256 over `method:path:timestamp:tenant:body_hash`.
  Timestamp tolerance = 300 seconds. Prevents replay attacks.
- **RBAC:** Hardcoded roles (admin, operator, viewer, api_key) with
  explicit permission lists. No dynamic policy evaluation.
- **Rate Limiting:** Sliding-window per `tenant_id` + `actor_id`.
  Stored in SQLite with `INSERT ... ON CONFLICT UPDATE`.
- **Secret Management:** Resolution priority: env var > file > default.
  `PI_SECRET_<NAME>` environment variables.
- **Audit Logging:** Every API action logged with hash-chained audit entries.
  Correlation ID propagated across all layers.

### 17.3 Telemetry Semantics

- **Structured Logging:** JSON output only. Every line includes
  `timestamp`, `component`, `correlation_id`, `level`, `event`.
- **Metrics Registry:** Prometheus-compatible text exposition format.
  Counter / Gauge / Histogram types. Thread-safe.
- **Distributed Tracing:** Custom lightweight tracer. Spans are immutable.
  Trace ID derived from `correlation_id`. No probabilistic sampling.
- **Health Checks:** `/v1/health/ready` returns HTTP 200 (healthy) or 503
  (degraded). Deterministic based on `health_records` table.

### 17.4 API Semantics

- **Versioning:** All routes under `/v1/` prefix.
- **Tenant Header:** `X-Tenant-ID` required on every request.
- **Correlation Header:** `X-Correlation-ID` optional; generated if absent.
- **Deterministic Headers:** Every response includes `X-PI-Version` and
  `X-PI-Deterministic: true`.
- **Rate Limit Response:** HTTP 429 with `Retry-After` logic.
- **Authentication Response:** HTTP 401 for missing/invalid token.
- **Authorization Response:** HTTP 403 for role/tenant mismatch.

### 17.5 Deployment Semantics

- **Docker:** Multi-stage build, non-root user (`piuser`), healthcheck,
  WAL SQLite volume mounted at `/data`.
- **Kubernetes:** Deployment (2 replicas), PVC (10Gi), Secret for JWT and
  signing keys. Liveness + readiness probes on `/v1/health/ready`.
- **Prometheus:** Optional sidecar profile in Docker Compose.
- **Backup:** SQLite file is single-file backup. WAL checkpoint before copy.

### 17.6 Fail-Closed Guarantees

- All new endpoints fail closed on auth failure, rate limit, or tenant
  inactivity.
- Append-only triggers prevent any mutation of historical data.
- Signature verification rejects requests outside timestamp tolerance.
- JWT expiry is strict — no grace period.
- Health endpoint returns 503 if any component is degraded.

---

## 18. Conformance Test Plan (Production)

### 18.1 Storage Tests

| ID | Description | Pass Criteria |
|----|-------------|---------------|
| S1 | Persist snapshot, retrieve latest | Row present, hash matches |
| S2 | Tenant isolation across writes | Cross-tenant reads return empty |
| S3 | Chain retrieval ordered | Sequence ASC, hash chain valid |
| S4 | Integrity verification | `verify_chain()` returns True |
| S5 | UPDATE blocked by trigger | `MUTATION_FORBIDDEN` error |
| S6 | DELETE blocked by trigger | `MUTATION_FORBIDDEN` error |
| S7 | Artifact hash is SHA-256 hex | Length 64, non-empty |

### 18.2 Security Tests

| ID | Description | Pass Criteria |
|----|-------------|---------------|
| SEC1 | JWT encode/decode roundtrip | Claims match, signature valid |
| SEC2 | Expired token rejected | `AuthenticationError` raised |
| SEC3 | Wrong secret rejected | `AuthenticationError` raised |
| SEC4 | Request sign/verify | HMAC matches |
| SEC5 | Old timestamp rejected | `SignatureError` raised |
| SEC6 | RBAC admin all permissions | `allowed()` True for any action |
| SEC7 | RBAC operator no cross-tenant | `allowed_tenants()` False |
| SEC8 | Secret env priority | Env value returned over file |
| SEC9 | Secret rotation generates valid | Alphanumeric + `_-`, length 64 |

### 18.3 Telemetry Tests

| ID | Description | Pass Criteria |
|----|-------------|---------------|
| TEL1 | Counter increment | Prometheus output contains value |
| TEL2 | Thread-safe counter | 10 threads x 100 increments = 1000 |
| TEL3 | Gauge set | Value appears in output |
| TEL4 | Histogram record | Count + sum correct |
| TEL5 | Structured log JSON | Valid JSON, correlation_id present |
| TEL6 | Tracer span duration | Positive after sleep |
| TEL7 | Telemetry scope error | Span status = ERROR |

### 18.4 Integration Tests

| ID | Description | Pass Criteria |
|----|-------------|---------------|
| INT1 | End-to-end storage + security + telemetry | All subsystems work together |
| INT2 | Tenant isolation full stack | No cross-tenant leakage |
| INT3 | Rate limiter enforcement | 4th request blocked after limit=3 |
| INT4 | Chain integrity under load | 10 snapshots, verify passes |
| INT5 | Audit hash chain under load | 20 entries, all hashes non-empty |
| INT6 | Concurrent writes safe | 10 threads, 10 rows, no corruption |
| INT7 | Combined JWT + signer | Both verify successfully |
...
- Every production change must be added to this spec before implementation.
- Test count must increase with every version bump.
- Zero regressions policy: all previous tests must pass.
- Determinism proofs must not change without major version bump.
