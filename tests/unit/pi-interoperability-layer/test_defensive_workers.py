...
def test_telemetry_governor_detects_debug_endpoint() -> None:
    ...
    trace_slot = bus.write(ArtifactSlot(
...
def test_telemetry_governor_detects_stack_trace_field() -> None:
    ...
    trace_slot = bus.write(ArtifactSlot(
        producer_worker_id="ext",
        artifact_type="SemanticIRTrace",
        payload={"traces": [{"endpoint_template": "/api/users", "method": "GET", "fields": [{"name": "error_stack_trace", "type": "string"}]}]},
    ).freeze())
...
def test_telemetry_governor_detects_token_leakage() -> None:
    ...
    trace_slot_2 = bus.write(ArtifactSlot(
        producer_worker_id="ext",
        artifact_type="SemanticIRTrace",
        payload={"traces": [{"endpoint_template": "/api/auth", "method": "POST", "fields": [{"name": "token", "description": "Bearer eyJhbGciOiJIUzI1NiIs.eyJzdWIiOiIxMjM0NTY3ODkwIiw.abc123"}]}]},
    ).freeze())
...
def test_replay_sanitizer_masks_sensitive_keys() -> None:
    bus = ArtifactBus()
    ledger = OrchestrationLedger(pipeline_name="test")
    raw_slot = bus.write(ArtifactSlot(
...
def test_replay_sanitizer_masks_jwt() -> None:
    ...
    raw_slot = bus.write(ArtifactSlot(
        producer_worker_id="src",
        artifact_type="RawSourceSnapshot",
        payload={"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKxwRJSMeKKF2QT4fwpMe"},
    ).freeze())
...
def test_sensitive_flow_tracker_detects_crossing() -> None:
    ...
    trace_slot = bus.write(ArtifactSlot(
        producer_worker_id="ext",
        artifact_type="SemanticIRTrace",
        payload={"traces": [{"endpoint_template": "/public/api/users", "method": "POST", "fields": [{"name": "password", "type": "string"}]}]},
    ).freeze())
    dep_slot = bus.write(ArtifactSlot(
        producer_worker_id="ext",
        artifact_type="DependencyGraph",
        payload={"edges": [{"upstream_endpoint": "/public/api/users", "downstream_endpoint": "/internal/svc-auth", "edge_type": "direct_call"}]},
    ).freeze())
...
def test_observability_diff_detects_verbosity_expansion() -> None:
    bus = ArtifactBus()
    ledger = OrchestrationLedger(pipeline_name="test")
    baseline = bus.write(ArtifactSlot(
    ...
    modified = bus.write(ArtifactSlot(
        producer_worker_id="obs",
        artifact_type="TelemetrySnapshot",
        payload={"log_lines": 300, "sensitive_fields": ["user_id", "email", "ssn"], "metadata_keys": ["timestamp", "request_id", "trace_id", "span_id"]},
    ).freeze())
...
def test_compliance_engine_detects_gdpr_missing_classification() -> None:
    ...
    trace_slot = bus.write(ArtifactSlot(
        producer_worker_id="ext",
        artifact_type="SemanticIRTrace",
        payload={"traces": [{"endpoint_template": "/api/users", "method": "POST", "fields": [{"name": "email", "type": "string", "data_classification": "PUBLIC"}]}]},
    ).freeze())
    worker = ComplianceEngineWorker("ce1", WorkerContract(worker_class="ComplianceEngineWorker"), bus, ledger)
    receipt = worker.execute("VALIDATE", [trace_slot.slot_id])
    assert receipt.status == "SUCCESS"
    out = bus.read(receipt.output_slot_ids[0])
    assert any(v["framework"] == "GDPR" and v["rule"] == "personal_data_classification" for v in out.payload["violations"])


def test_compliance_engine_evaluates_soc2_boundary_failure() -> None:
    bus = ArtifactBus()
    ledger = OrchestrationLedger(pipeline_name="test")
    boundary_slot = bus.write(ArtifactSlot(
...
def test_security_simulation_validates_replay_containment() -> None:
    bus = ArtifactBus()
    ledger = OrchestrationLedger(pipeline_name="test")
    sanitized = bus.write(ArtifactSlot(
...
def test_render_sensitive_lineage_html() -> None:
    with tempfile.NamedTemporaryFile(suffix=".html", delete=False, mode="w") as f:
        pass
    render_sensitive_lineage({"trust_boundary_crossings": [{"edge": "A->B", "from_zone": "TRUSTED", "to_zone": "UNTRUSTED", "sensitive_fields_crossed": ["password"]}]}, f.name)
    content = Path(f.name).read_text()
...
def test_render_replay_sanitization_html() -> None:
    with tempfile.NamedTemporaryFile(suffix=".html", delete=False, mode="w") as f:
        pass
    render_replay_sanitization({"redaction_log": [{"path": "token", "rule": "jwt_masking", "mask": "MASKED_abc"}], "replay_equivalence_preserved": True}, f.name)
    content = Path(f.name).read_text()
...
def test_render_compliance_violations_html() -> None:
    with tempfile.NamedTemporaryFile(suffix=".html", delete=False, mode="w") as f:
        pass
    render_compliance_violations({"violations": [{"framework": "GDPR", "rule": "r1", "detail": "d1"}], "frameworks_evaluated": ["GDPR"]}, f.name)
    content = Path(f.name).read_text()
...
def test_render_observability_drift_html() -> None:
    with tempfile.NamedTemporaryFile(suffix=".html", delete=False, mode="w") as f:
        pass
    render_observability_drift({"findings": [{"rule": "verbosity_expansion", "detail": "Lines increased by 500"}], "drift_score": 1, "new_sensitive_fields": ["email"]}, f.name)
    content = Path(f.name).read_text()
...
def test_query_secret_exposure() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        reg.store_snapshot("TelemetryGovernorWorker", "exec_1", {"findings": [{"rule": "token_leakage", "severity": "CRITICAL"}, {"rule": "debug_endpoint_exposure", "severity": "HIGH"}]})
...
def test_query_sensitive_paths() -> None:
    with tempfile.TemporaryDirectory() as td:
        reg = SnapshotRegistry(root_dir=Path(td))
        reg.store_snapshot("SensitiveFlowTrackerWorker", "exec_1", {"field_propagation": [{"field": "password", "endpoint": "/api/users", "event": "origin", "trust_zone": "TRUSTED"}]})
        snaps = reg.list_snapshots()
        snap_id = snaps[0].snapshot_id
        engine = SemanticQueryEngine(reg)
...
def test_query_observability_drift() -> None:
    with tempfile.TemporaryDirectory() as td:
        reg = SnapshotRegistry(root_dir=Path(td))
        reg.store_snapshot("ObservabilityDiffWorker", "exec_1", {"findings": [{"rule": "verbosity_expansion"}], "verbosity_expansion": 250, "new_sensitive_fields": ["email"], "drift_score": 2})
        snaps = reg.list_snapshots()
        snap_id = snaps[0].snapshot_id
        engine = SemanticQueryEngine(reg)
...
def test_query_compliance_violations() -> None:
    with tempfile.TemporaryDirectory() as td:
        reg = SnapshotRegistry(root_dir=Path(td))
        reg.store_snapshot("ComplianceEngineWorker", "exec_1", {"violations": [{"framework": "GDPR", "rule": "r1", "detail": "d1"}]})
        snaps = reg.list_snapshots()
        snap_id = snaps[0].snapshot_id
        engine = SemanticQueryEngine(reg)
