...
def test_entropy_monitor_monotonicity():
    ...
    from pi_agent_chain.models import SemanticField, SemanticIRTrace
...
def test_replay_validator_canonicalization_strips_timestamps():
    ...
    orig = SemanticIRTrace(
        ...
        fields=[
            SemanticField(
                path="body.name", inferred_type="STRING", confidence=0.95, entropy_score=0.3, example_value="Alice"
            ),
        ],
        is_frozen=True,
        frozen_at=now,
    ...
    replay = SemanticIRTrace(
        ...
        fields=[
            SemanticField(
                path="body.name", inferred_type="STRING", confidence=0.95, entropy_score=0.3, example_value="Bob"
            ),
        ],
        is_frozen=True,
...
def test_auth_consistency_auth_transition_401():
    ...
    traces = [SemanticIRTrace(endpoint_template="/admin", method="GET", fields=[]) for _ in range(5)]
...
def test_state_transition_fsm_basic():
    from pi_agent_chain.models import AuthConsistencyReport, EpistemicState, NormalizedTrafficPacket, SemanticIRTrace
...
def test_state_transition_fsm_bounds():
    ...
    traces = [SemanticIRTrace(endpoint_template=f"/endpoint{i}", method="GET", fields=[]) for i in range(100)]
    packets = [
...
def test_semantic_quorum_contested_collision():
    ...
    a1 = SemanticArtifact(
        ...
        semantic_hash="h1",
        generated_by="SemanticTyperNode",
        payload_json=json.dumps(trace1.model_dump()),
        provenance=[],
    )
    a2 = SemanticArtifact(
        artifact_id="a2",
        ...
        semantic_hash="h2",
        generated_by="SemanticTyperNode",
        payload_json=json.dumps(trace2.model_dump()),
        provenance=[],
    )
...
def test_semantic_quorum_overflow_protection():
    ...
    for i in range(550):
        trace = SemanticIRTrace(
            endpoint_template=f"/api/v1/endpoint{i}",
            method="GET",
...
def test_entropy_analysis_structural_measurement():
    ...
    claim = SemanticClaim(
        claim_id="c1",
        property_path="body.id",
        semantic_type="UUIDv4",
        confidence_score=0.95,
        artifact_id="a1",
        trace_id="t1",
        packet_id="",
        worker_id="w1",
        source_epistemic_state=EpistemicState.OBSERVED,
    ...
    quorum = SemanticQuorumReport(
        ...
        intersections=[
            SemanticIntersection(
                intersection_id="i1",
                property_path="body.id",
                intersected_type="UUIDv4",
                intersected_confidence=0.95,
                agreement_claim_ids=["c1"],
                total_authority_sum=1.0,
    ...
    auth = AuthConsistencyReport(
        ...
        evidence=[
            AuthEvidence(
                ...
                trace_id="t1",
                packet_id="p1",
                evidence_type="BEARER_HEADER",
                field_path="header.Authorization",
                carrier="HEADER",
                observed_value_hash="abc",
                status_code=200,
            )
        ],
        violations=[
            GovernanceViolation(
                violation_id="v1",
                rule="AUTH_INVARIANT_BROKEN",
                worker_id="auth_validator",
                root_goal_id="e1",
                severity="ERROR",
                context={},
                action_taken="HALT",
...
def test_entropy_analysis_topological_from_fsm():
    ...
    fsm = ProtocolStateMachine(
        ...
        nodes=[
            ...
            StateNode(
                node_id="n2", endpoint_template="/profile", method="GET", epistemic_state=EpistemicState.OBSERVED
            ),
        ],
        edges=[
            TransitionEdge(
...
def test_entropy_analysis_convergence_scoring():
    ...
    c = SemanticClaim(
        ...
        packet_id="",
        worker_id="w1",
        source_epistemic_state=EpistemicState.REPLAY_CONFIRMED,
        replay_confirmed=True,
        provenance_chain=["a1"],
        authority_weight=1.0,
    )
