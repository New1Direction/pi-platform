...
from pi_agent_interceptor.proxy import app, ledger, require_operator
...
def _operator_token():
    """Marketplace endpoints are gated by require_operator; override the
    dep so tests don't need to send X-PI-Operator-Token."""
...
MOCK_HEADERS = {"x-ratelimit-remaining-tokens": "750000", "x-ratelimit-remaining": "750000"}
...
def test_dynamic_bundle_creation():
    ...
    bundle = orchestrator.create_surplus_bundle(
        name="100k-tokens-test", token_cap=100000, price=5.0, expires_in_sec=3600
    )
...
def test_immutable_audit_trail_generation():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT payload_json FROM audit_events WHERE request_type = 'SURPLUS_BUNDLE_SALE' ORDER BY id DESC LIMIT 1;"
        )
        row = cursor.fetchone()
...
def test_governor_static_surplus_screening(tmp_path):
    ...
    bundle = ExtensionBundle(bundle_id="b_surplus_test", manifest=manifest, payload_hash="ph_surplus")
...
def test_warn_only_mode(monkeypatch, tmp_path):
    ...
    bundle = ExtensionBundle(bundle_id="b_warn_test", manifest=manifest, payload_hash="ph_warn")
...
"""
    ...
    class FakeSandbox:
        def verify_determinism(self, *args, **kwargs):
            return True
...
def test_marketplace_api_endpoints():
    ...
    buy_payload = {"name": "Marketplace Special", "token_cap": 50000, "price": 2.50, "expires_in_sec": 300}
