...
def test_constant_time_auditor_direct(monkeypatch):
    ...
    res_safe = agent.audit_constant_time(ConstantTimeInput(
        file_path="crypto.py",
        source_code=code_safe,
        secrets_context=["secret_key"]
    ))
    ...
    res_vuln = agent.audit_constant_time(ConstantTimeInput(
        file_path="crypto.py",
        source_code=code_vuln,
        secrets_context=["secret_key"]
    ))
    ...
    res_warn = agent.audit_constant_time(ConstantTimeInput(
        file_path="crypto.py",
        source_code=code_vuln,
        secrets_context=["secret_key"]
    ))
...
def test_memory_zeroize_sentry_direct(monkeypatch):
    ...
    res_safe = agent.audit_memory_zeroize(MemoryZeroizeInput(
        file_path="sec_buffer.c",
        source_code=code_safe,
    ...
    res_vuln = agent.audit_memory_zeroize(MemoryZeroizeInput(
        file_path="sec_buffer.c",
        source_code=code_vuln,
        sensitive_symbols=["password"]
    ))
    ...
    res_warn = agent.audit_memory_zeroize(MemoryZeroizeInput(
        file_path="sec_buffer.c",
        source_code=code_vuln,
        sensitive_symbols=["password"]
    ))
...
def test_dimensional_analysis_sentry_direct(monkeypatch):
    ...
    res_safe = agent.audit_dimensions(DimensionalAnalysisInput(
        file_path="finance.py",
        source_code=code_safe,
        unit_registry={"balance_a": "wei", "fee_a": "wei"}
    ))
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
    ...
    res_vuln = agent.audit_dimensions(DimensionalAnalysisInput(
        file_path="finance.py",
        source_code=code_vuln,
        unit_registry={"balance_a": "wei", "rate_b": "gwei"}
    ))
    assert not res_vuln.is_secure
    ...
    res_warn = agent.audit_dimensions(DimensionalAnalysisInput(
        file_path="finance.py",
        source_code=code_vuln,
        unit_registry={"balance_a": "wei", "rate_b": "gwei"}
    ))
...
def test_agent_tool_execution_guard_direct(monkeypatch):
    ...
    # Safe
    res_safe = agent.audit_agent_command(AgentToolGuardInput(
        command_string="pytest tests/integration/test_cloned_skills_agents.py",
    ...
    res_vuln = agent.audit_agent_command(AgentToolGuardInput(
        command_string="rm -rf /Users/clubpenguin/Downloads",
        allowed_commands=["rm", "pytest"]
    ))
    assert not res_vuln.is_secure
    ...
    res_warn = agent.audit_agent_command(AgentToolGuardInput(
        command_string="rm -rf /Users/clubpenguin/Downloads",
        allowed_commands=["rm", "pytest"]
    ))
    assert res_warn.is_secure
...
def test_hot_path_allocation_auditor_direct(monkeypatch):
    ...
    res_safe = agent.audit_hot_path(HotPathAllocationInput(
    ...
    res_vuln = agent.audit_hot_path(HotPathAllocationInput(
        file_path="program.cs",
        source_code=code_vuln,
        hot_path_lines=[1]
    ))
    ...
    res_warn = agent.audit_hot_path(HotPathAllocationInput(
        file_path="program.cs",
        source_code=code_vuln,
        hot_path_lines=[1]
    ))
...
def test_orchestrator_routing_memory_zeroize(monkeypatch):
    ...
    context = {
        "file_path": "sec_buffer.c",
        "content": "memset(my_secret, 0, 32);",
        "sensitive_symbols": ["my_secret"]
    }
    inp = OrchestratorInput(goal=goal, context=context)
    out = orchestrator.execute_goal(inp)
