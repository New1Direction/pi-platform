"""Integration tests for the PiArithmeticAuditor micro-agent and its Orchestrator consensus execution."""

from __future__ import annotations

import os
import pytest

from pi_micro_agents.pi_arithmetic_auditor import (
    PiArithmeticAuditor,
    ArithmeticInput,
    ArithmeticOutput,
    is_strict_mode,
)
from pi_micro_agents.pi_orchestrator import PiOrchestrator, OrchestratorInput


@pytest.fixture(autouse=True)
def clean_env(monkeypatch):
    """Ensure clean environment variable setup for each test run."""
    monkeypatch.delenv("PI_ARITHMETIC_STRICT_MODE", raising=False)
    monkeypatch.delenv("PI_ORCHESTRATOR_STRICT_MODE", raising=False)


# =====================================================================
# Test 1: Legacy Solidity Contract with Vulnerable Raw Math (pre-0.8)
# =====================================================================
def test_arithmetic_legacy_vulnerable_contract():
    """Verify that a legacy Solidity contract using raw math operations is flagged."""
    agent = PiArithmeticAuditor()

    solidity_code = """
    pragma solidity ^0.7.0;

    contract LegacyToken {
        mapping(address => uint256) public balances;

        function transfer(address to, uint256 amount) public {
            // Raw arithmetic operations without SafeMath (vulnerable to overflow/underflow)
            balances[msg.sender] = balances[msg.sender] - amount;
            balances[to] = balances[to] + amount;
        }
    }
    """
    inp = ArithmeticInput(
        file_path="LegacyToken.sol",
        solidity_code=solidity_code,
        check_level="STRICT"
    )

    out = agent.audit_arithmetic(inp)

    assert out.is_secure is False
    assert "transfer" in out.vulnerable_functions
    assert out.risk_score == 95.0
    assert out.status == "REJECTED_ARITHMETIC_VULNERABILITY"
    assert len(out.flagged_findings) >= 1
    assert "without incorporating SafeMath libraries" in out.flagged_findings[0]


# =====================================================================
# Test 2: Legacy Solidity Contract utilizing SafeMath (pre-0.8)
# =====================================================================
def test_arithmetic_legacy_safemath_contract():
    """Verify that a legacy Solidity contract using SafeMath successfully passes."""
    agent = PiArithmeticAuditor()

    solidity_code = """
    pragma solidity ^0.7.0;

    import "@openzeppelin/contracts/math/SafeMath.sol";

    contract SafeLegacyToken {
        using SafeMath for uint256;
        mapping(address => uint256) public balances;

        function transfer(address to, uint256 amount) public {
            // Uses SafeMath methods to prevent overflow/underflow
            balances[msg.sender] = balances[msg.sender].sub(amount);
            balances[to] = balances[to].add(amount);
        }
    }
    """
    inp = ArithmeticInput(
        file_path="SafeLegacyToken.sol",
        solidity_code=solidity_code,
        check_level="STRICT"
    )

    out = agent.audit_arithmetic(inp)

    assert out.is_secure is True
    assert out.risk_score == 0.0
    assert out.status == "PASSED"
    assert len(out.vulnerable_functions) == 0


# =====================================================================
# Test 3: Modern Auto-Safe Solidity Contract (>=0.8.0)
# =====================================================================
def test_arithmetic_modern_safe_contract():
    """Verify that modern Solidity contracts with default bounds check pass."""
    agent = PiArithmeticAuditor()

    solidity_code = """
    pragma solidity ^0.8.0;

    contract ModernToken {
        mapping(address => uint256) public balances;

        function transfer(address to, uint256 amount) public {
            // Under 0.8.0+, raw operations automatically revert on overflow/underflow
            balances[msg.sender] = balances[msg.sender] - amount;
            balances[to] = balances[to] + amount;
        }
    }
    """
    inp = ArithmeticInput(
        file_path="ModernToken.sol",
        solidity_code=solidity_code,
        check_level="STRICT"
    )

    out = agent.audit_arithmetic(inp)

    assert out.is_secure is True
    assert out.risk_score == 0.0
    assert out.status == "PASSED"


# =====================================================================
# Test 4: Modern Solidity Contract with Unchecked Block Warning
# =====================================================================
def test_arithmetic_modern_unchecked_warning():
    """Verify that modern Solidity contracts utilizing unchecked blocks trigger safety warnings."""
    agent = PiArithmeticAuditor()

    solidity_code = """
    pragma solidity ^0.8.0;

    contract ModernUncheckedToken {
        mapping(address => uint256) public balances;

        function transfer(address to, uint256 amount) public {
            unchecked {
                // Unchecked block disables Solidity 0.8+ compiler safety checks
                balances[msg.sender] = balances[msg.sender] - amount;
                balances[to] = balances[to] + amount;
            }
        }
    }
    """
    inp = ArithmeticInput(
        file_path="ModernUncheckedToken.sol",
        solidity_code=solidity_code,
        check_level="STRICT"
    )

    out = agent.audit_arithmetic(inp)

    assert out.is_secure is False
    assert "transfer" in out.vulnerable_functions
    assert out.risk_score == 95.0
    assert out.status == "REJECTED_ARITHMETIC_VULNERABILITY"
    assert len(out.flagged_findings) >= 1
    assert "unchecked" in out.flagged_findings[0]


# =====================================================================
# Test 5: Orchestrator Routing & Consensus Passed
# =====================================================================
def test_orchestrator_arithmetic_consensus_passed(monkeypatch):
    """Verify NLP intent routing to PiArithmeticAuditor resolves low-divergence majority votes."""
    monkeypatch.setenv("PI_ARITHMETIC_STRICT_MODE", "true")
    monkeypatch.setenv("PI_ORCHESTRATOR_STRICT_MODE", "true")

    orchestrator = PiOrchestrator()

    solidity_code = """
    pragma solidity ^0.7.0;
    contract Legacy {
        function add(uint a, uint b) public {
            uint c = a + b;
        }
    }
    """

    mock_consensus_runs = [
        {
            "is_secure": False,
            "vulnerable_functions": ["add"],
            "flagged_findings": ["Raw arithmetic operator on Line 4"],
            "risk_score": 95.0,
            "status": "REJECTED_ARITHMETIC_VULNERABILITY"
        },
        {
            "is_secure": False,
            "vulnerable_functions": ["add"],
            "flagged_findings": ["Raw arithmetic operator on Line 4"],
            "risk_score": 95.0,
            "status": "REJECTED_ARITHMETIC_VULNERABILITY"
        },
        {
            "is_secure": False,
            "vulnerable_functions": ["add"],
            "flagged_findings": ["Raw arithmetic operator on Line 4"],
            "risk_score": 95.0,
            "status": "REJECTED_ARITHMETIC_VULNERABILITY"
        }
    ]

    inp = OrchestratorInput(
        goal="arithmetic audit on Legacy.sol to scan for legacy math flaws",
        context={
            "file_path": "Legacy.sol",
            "solidity_code": solidity_code,
            "mock_consensus_runs": mock_consensus_runs
        }
    )
    res = orchestrator.execute_goal(inp)

    assert res.success is False
    assert res.risk_score == 95.0
    assert "PiArithmeticAuditor" in res.routed_agent
    assert "Consensus Passed" in res.output_summary
    assert "REJECTED_ARITHMETIC_VULNERABILITY" in res.output_summary
    assert "consensus_telemetry" in res.result_details
    assert res.result_details["consensus_telemetry"]["status"] == "CONSENSUS_PASSED"


# =====================================================================
# Test 6: Orchestrator Consensus High Divergence / Split Vote
# =====================================================================
def test_orchestrator_arithmetic_consensus_failed_divergence(monkeypatch):
    """Verify that split vote high-divergence output triggers fail-shut and blocks execution."""
    monkeypatch.setenv("PI_ORCHESTRATOR_STRICT_MODE", "true")

    from pi_semantic_radius.consensus_breaker import PiConsensusBreaker, DivergenceReport
    def mock_evaluate_consensus(self, prompt, responses):
        return DivergenceReport(
            prompt=prompt,
            responses=responses,
            semantic_divergence=85.0,
            structural_divergence=0.0,
            consensus_divergence_score=85.0,
            is_broken=True
        )
    monkeypatch.setattr(PiConsensusBreaker, "evaluate_consensus", mock_evaluate_consensus)

    orchestrator = PiOrchestrator()

    solidity_code = """
    pragma solidity ^0.7.0;
    contract Legacy {
        function add(uint a, uint b) public {
            uint c = a + b;
        }
    }
    """

    mock_consensus_runs = [
        {
            "is_secure": True,
            "vulnerable_functions": [],
            "flagged_findings": [],
            "risk_score": 0.0,
            "status": "PASSED"
        },
        {
            "is_secure": False,
            "vulnerable_functions": ["add"],
            "flagged_findings": ["Raw arithmetic operator on Line 4"],
            "risk_score": 95.0,
            "status": "REJECTED_ARITHMETIC_VULNERABILITY"
        },
        {
            "is_secure": True,
            "vulnerable_functions": [],
            "flagged_findings": [],
            "risk_score": 0.0,
            "status": "PASSED"
        }
    ]

    inp = OrchestratorInput(
        goal="arithmetic audit on Legacy.sol",
        context={
            "file_path": "Legacy.sol",
            "solidity_code": solidity_code,
            "mock_consensus_runs": mock_consensus_runs
        }
    )
    res = orchestrator.execute_goal(inp)

    assert res.success is False
    assert "consensus coordination alarm" in res.output_summary or "REJECTED_DIVERGENCE_ALARM" in res.output_summary
    assert "consensus_telemetry" in res.result_details
    assert res.result_details["consensus_telemetry"]["status"] == "REJECTED_DIVERGENCE_ALARM"
    assert res.result_details["consensus_telemetry"]["divergence_score"] >= 60.0
