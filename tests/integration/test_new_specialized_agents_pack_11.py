...
from pi_micro_agents import (
    AstDepthInput,
    CavemanCompressorInput,
    ChangelogInput,
    CommitLinterInput,
    DeadCodeInput,
    DepreciationInput,
    DesignAnInterfaceInput,
    ErrorCatchInput,
    GitSafetyInput,
    GrillMeInput,
    HandoffInput,
...
def test_handoff_checkpoint_sentry(monkeypatch):
    ...
    res_safe = agent.audit_handoff(
        HandoffInput(handoff_content="Status is green. Reproduction: run pytest. Next steps: release.")
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_to_prd_validator(monkeypatch):
    ...
    res_safe = agent.validate_prd(
        ToPrdInput(
            prd_content="Objectives: build website. Specifications: use html. Non-goals: hosting. Validation: run lighthouse."
        )
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_to_issues_breakdown(monkeypatch):
    ...
    res_safe = agent.breakdown_issues(
        ToIssuesInput(
            spec_content="Task 1: Code backend. Checklist: - create db. Acceptance Criteria: server responds with 200."
        )
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_tdd_test_file_verifier(monkeypatch):
    ...
    res_vuln = agent.verify_test_files(
        TddTestFileInput(modified_files=["src/auth.py", "src/models.py", "tests/test_auth.py"])
    )
    assert not res_vuln.is_secure
    ...
    res_warn = agent.verify_test_files(
        TddTestFileInput(modified_files=["src/auth.py", "src/models.py", "tests/test_auth.py"])
    )
    assert res_warn.is_secure
...
def test_architecture_import_boundary_sentry(monkeypatch):
    ...
    res_vuln = agent.check_import_boundaries(
        ImportBoundaryInput(
    ...
    res_warn = agent.check_import_boundaries(
        ImportBoundaryInput(
            file_path="app/infrastructure/db.py",
            code_content=code_vuln,
            forbidden_mappings={"app/infrastructure": ["app/domain"]},
    ...
    res_safe = agent.check_import_boundaries(
        ImportBoundaryInput(
            file_path="app/infrastructure/db.py",
...
def test_depreciation_scanner(monkeypatch):
    ...
    res_vuln = agent.scan_depreciation(
        DepreciationInput(file_path="app.py", code_content=code_vuln, deprecated_patterns=["import imp"])
    )
    assert not res_vuln.is_secure
    ...
    res_warn = agent.scan_depreciation(
        DepreciationInput(file_path="app.py", code_content=code_vuln, deprecated_patterns=["import imp"])
    )
    assert res_warn.is_secure
    assert res_warn.status == "WARN_DEPRECIATION"

    res_safe = agent.scan_depreciation(
        DepreciationInput(file_path="app.py", code_content="import importlib", deprecated_patterns=["import imp"])
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_mock_data_tainting_sentry(monkeypatch):
    ...
    res_safe = agent.check_mock_tainting(
        MockDataTaintingInput(file_path="mocks.js", data_content="let key = 'mock_key_value';")
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_readme_validator(monkeypatch):
    ...
    res_safe = agent.validate_readme(
        ReadmeInput(
...
def test_changelog_auditor(monkeypatch):
    ...
    res_vuln = agent.audit_changelog(
        ChangelogInput(changelog_content="# Changelog\n## [0.9.0]\n- initial release", target_version="1.0.0")
    )
    assert not res_vuln.is_secure
    assert res_vuln.status == "REJECTED_CHANGELOG"

    monkeypatch.setenv("PI_CHANGELOG_STRICT_MODE", "false")
    res_warn = agent.audit_changelog(
        ChangelogInput(changelog_content="# Changelog\n## [0.9.0]\n- initial release", target_version="1.0.0")
    )
    assert res_warn.is_secure
    assert res_warn.status == "WARN_CHANGELOG"

    res_safe = agent.audit_changelog(
        ChangelogInput(changelog_content="# Changelog\n## [1.0.0]\n- initial release", target_version="1.0.0")
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def deeply_nested():
            ...
            if True:
                if True:
                    if True:
                        pass
    """
    res_vuln = agent.check_ast_depth(AstDepthInput(file_path="app.py", code_content=nested_code, max_depth=4))
    assert not res_vuln.is_secure
    ...
    res_warn = agent.check_ast_depth(AstDepthInput(file_path="app.py", code_content=nested_code, max_depth=4))
    assert res_warn.is_secure
...
def flat():
    if True:
        pass
    """
    res_safe = agent.check_ast_depth(AstDepthInput(file_path="app.py", code_content=safe_code, max_depth=4))
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
...
def test_semantic_commit_message_linter(monkeypatch):
    ...
    res_safe = agent.audit_commit_message(
        CommitLinterInput(commit_message="fix(router): correct keyword lookup registry")
    )
    assert res_safe.is_secure
    assert res_safe.status == "PASSED"
