...
class AgentToolGuardInput(BaseModel):
    ...
    allowed_commands: List[str] = Field(
        default_factory=lambda: ["git", "pytest", "ruff", "python"], description="Prefixes of allowed commands"
    )
...
class PiAgentToolExecutionGuard:
    ...
    def audit_agent_command(self, input_envelope: AgentToolGuardInput) -> AgentToolGuardOutput:
        ...
        return AgentToolGuardOutput(is_secure=is_secure, blocked_patterns=blocked, risk_score=risk_score, status=status)
