...
class UnboundedLoopsStateOutput(BaseModel):
    ...
    flagged_findings: List[str] = Field(default_factory=list, description="Detailed findings on unbounded loops with state updates")
            ...
            for loop_str, condition in loops:
                ...
                if is_unbounded:
                    ...
                    if loop_body_match:
                        ...
                        assignments = re.findall(r'\b([a-zA-Z0-9_]+)(?:\s*\[.*?\]|\s*\.[a-zA-Z0-9_]+)*\s*[-+*\/]?=(?!=)', loop_body)
                        non_loop_assignments = [asg for asg in assignments if asg.strip() not in ["i", "j", "k", "index", "iter"]]
