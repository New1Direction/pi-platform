...
class GrillMeOutput(BaseModel):
    ...
    missing_prerequisites: List[str] = Field(
        default_factory=list, description="List of missing details or vague phrases"
    )
...
class PiGrillMeQuestionnaire:
    ...
    def grill_plan(self, input_envelope: GrillMeInput) -> GrillMeOutput:
        ...
        return GrillMeOutput(is_secure=is_secure, missing_prerequisites=missing, risk_score=risk_score, status=status)
