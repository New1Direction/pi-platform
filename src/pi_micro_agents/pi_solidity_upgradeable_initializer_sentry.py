...
class UpgradeableInitOutput(BaseModel):
    ...
    flagged_findings: List[str] = Field(
        default_factory=list, description="Detailed upgradeable initialization findings"
    )
    risk_score: float = Field(..., description="Risk score from 0.0 to 100.0")
    status: str = Field(
        ..., description="Status classification (PASSED, WARN_INITIALIZER_RISK, REJECTED_INITIALIZER_RISK)"
    )
