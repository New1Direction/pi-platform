...
class CentralizationOutput(BaseModel):
    is_secure: bool = Field(..., description="Indicates if contract admin operations have appropriate decentralized protections")
    ...
    status: str = Field(..., description="Status classification (PASSED, WARN_CENTRALIZATION_RISK, REJECTED_CENTRALIZATION_RISK)")
        ...
        for func_name, func_body, start_line in functions:
            ...
            if any(action in func_name.lower() for action in ["mint", "pause", "unpause", "fee", "withdraw"]):
                if any(mod in func_body for mod in ["onlyOwner", "onlyAdmin", "onlyRole"]):
                    ...
                    if not any(safe in cleaned_body.lower() for safe in ["timelock", "delay", "propose", "execute", "multisig", "threshold"]):
