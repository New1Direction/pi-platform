...
class BlockTimestampOutput(BaseModel):
    ...
    flagged_findings: List[str] = Field(default_factory=list, description="Detailed block.timestamp and expiration findings")
    risk_score: float = Field(..., description="Risk score from 0.0 to 100.0")
    status: str = Field(..., description="Status classification (PASSED, WARN_TIMESTAMP_VULNERABILITY, REJECTED_TIMESTAMP_VULNERABILITY)")
