...
class TddTestFileOutput(BaseModel):
    ...
    vulnerable_files: List[str] = Field(default_factory=list, description="Implementation files lacking matching test files")
...
class PiTddTestFileVerifier:
    ...
    def verify_test_files(self, input_envelope: TddTestFileInput) -> TddTestFileOutput:
        ...
        for f in files:
            ...
            has_test = False
            for tf in files:
                if "test" in tf.lower() and (expected_test_py in tf or expected_test_ts in tf or base_name.split(".")[0] in tf):
                    has_test = True
                    break
