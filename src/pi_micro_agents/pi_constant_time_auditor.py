...
class ConstantTimeOutput(BaseModel):
    ...
    flagged_lines: List[str] = Field(
        default_factory=list, description="Flagged lines containing division or branching on secrets"
    )
...
class PiConstantTimeAuditor:
    ...
    def audit_constant_time(self, input_envelope: ConstantTimeInput) -> ConstantTimeOutput:
                ...
                if secret in line:
                    if "/" in line or "%" in line:
                        flagged_lines.append(
                            f"L{idx}: Potential secret-dependent division/modulo on '{secret}': {line.strip()}"
                        )
                    if re.search(r"\bif\s*\(.*" + re.escape(secret) + r".*\)", line) or re.search(
                        r"\bwhile\s*\(.*" + re.escape(secret) + r".*\)", line
                    ):
                        flagged_lines.append(
                            f"L{idx}: Potential secret-dependent branch/loop condition on '{secret}': {line.strip()}"
                        )
        ...
        return ConstantTimeOutput(
            is_secure=is_secure, flagged_lines=flagged_lines, risk_score=risk_score, status=status
        )
