...
class ShadowedVariableOutput(BaseModel):
    is_secure: bool = Field(..., description="Indicates if contract is free from variable shadowing and unused variable issues")
    vulnerable_functions: List[str] = Field(default_factory=list, description="Vulnerable function names")
    flagged_findings: List[str] = Field(default_factory=list, description="Detailed shadowing and unused variable findings")
    risk_score: float = Field(..., description="Risk score from 0.0 to 100.0")
    status: str = Field(..., description="Status classification (PASSED, WARN_SHADOW_VULNERABILITY, REJECTED_SHADOW_VULNERABILITY)")
...
class PiShadowedVariableDetector:
    ...
    def audit_shadowed(self, input_envelope: ShadowedVariableInput) -> ShadowedVariableOutput:
        ...
        state_var_pattern = re.compile(r'\b(address|uint256|bytes32|bool|string)\b\s+(?:public|private|internal)?\s*(?!constant|immutable)([a-zA-Z0-9_]+)\s*;')
            ...
            if param_match:
                ...
                for rp in raw_params:
                    ...
                    if parts:
                        ...
                        if var_name.startswith("memory") or var_name.startswith("calldata") or var_name.startswith("storage"):
                            if len(parts) >= 2:
