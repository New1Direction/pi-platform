...
class OracleLivenessOutput(BaseModel):
    ...
    status: str = Field(..., description="Status classification (PASSED, WARN_ORACLE_LIVENESS, REJECTED_ORACLE_LIVENESS)")
    ...
    def audit_oracle_liveness(self, input_envelope: OracleLivenessInput) -> OracleLivenessOutput:
        ...
        for name, args, body in func_blocks:
            if "latestRoundData" in body:
                ...
                has_freshness_check = re.search(r'block\.timestamp\s*-\s*updatedAt', body) or \
                                      re.search(r'updatedAt\s*-\s*block\.timestamp', body) or \
                                      re.search(r'require\s*\(\s*updatedAt\s*>\s*0\s*\)', body) or \
                                      re.search(r'require\s*\(\s*updatedAt\s*!=\s*0\s*\)', body)

                has_answer_validation = re.search(r'answer\s*>\s*0', body) or \
                                        re.search(r'price\s*>\s*0', body)
