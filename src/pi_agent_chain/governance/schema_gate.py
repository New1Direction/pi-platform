...
class SchemaGate:
    ...
    def validate(
        ...
        except _SchemaViolationError as exc:
    ...
    def _check_required(schema: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        for key in required:
            if key not in payload:
                raise _SchemaViolationError(key, f"Missing required field '{key}'")
        ...
        for key, value in payload.items():
            if key in properties:
                ...
                if expected == "array" and not isinstance(value, list):
                    raise _SchemaViolationError(key, f"Expected array, got {type(value).__name__}")
                if expected == "object" and not isinstance(value, dict):
                    raise _SchemaViolationError(key, f"Expected object, got {type(value).__name__}")
                if expected == "string" and not isinstance(value, str):
                    raise _SchemaViolationError(key, f"Expected string, got {type(value).__name__}")
                if expected == "integer" and not isinstance(value, int):
                    raise _SchemaViolationError(key, f"Expected integer, got {type(value).__name__}")
                if expected == "boolean" and not isinstance(value, bool):
                    raise _SchemaViolationError(key, f"Expected boolean, got {type(value).__name__}")
                if expected == "number" and not isinstance(value, (int, float)):
                    raise _SchemaViolationError(key, f"Expected number, got {type(value).__name__}")
